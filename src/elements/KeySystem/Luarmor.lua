...
local function LuarmorKeySystem(ks, GuiConfig, CoreGui, TweenService, getIconId, ConfigFolder)
    ...
    local function checkKey(key)
        ...
        if not lm then warn("[VelarisUI/Luarmor] Object tidak ditemukan di _G.") return false end
        local fn = lm.CheckKey or lm.checkKey or lm.check or lm.Validate or lm.validate
        if type(fn) ~= "function" then warn("[VelarisUI/Luarmor] Method CheckKey tidak ditemukan.") return false end
        local ok, result = pcall(fn, lm, key)
        return ok and result == true
    end

    local keyFileName = (type(ks.KeyFile) == "string" and ks.KeyFile ~= "") and ks.KeyFile or "CHX_key"
    ...
    Badge.BackgroundColor3 = Color3.fromRGB(90, 60, 180)
    ...
    BadgeLabel.Text = "LUARMOR"
    BadgeLabel.TextColor3 = Color3.fromRGB(220, 210, 255)
