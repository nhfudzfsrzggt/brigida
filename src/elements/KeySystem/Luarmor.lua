...
local function LuarmorKeySystem(ks, GuiConfig, CoreGui, TweenService, getIconId, ConfigFolder)
    ...
    local function luarmorCheckKey(key)
        ...
        if not lm then
            warn("[VelarisUI/Luarmor] Object tidak ditemukan di _G.")
            return false
        end
        local fn = lm.CheckKey or lm.checkKey or lm.check or lm.Validate or lm.validate
        if type(fn) ~= "function" then
            warn("[VelarisUI/Luarmor] Method CheckKey tidak ditemukan.")
            return false
        end
        local ok, result = pcall(fn, lm, key)
        if not ok then
            warn("[VelarisUI/Luarmor] CheckKey error: " .. tostring(result))
            return false
        end
        return result == true
    end
    ...
    LuarmorBadge.Size = UDim2.new(0, 0, 0, 18)
    LuarmorBadge.AutomaticSize = Enum.AutomaticSize.X
    LuarmorBadge.BackgroundColor3 = Color3.fromRGB(90, 60, 180)
    LuarmorBadge.BorderSizePixel = 0
    LuarmorBadge.ZIndex = 104
    LuarmorBadge.Parent = Card
    local LBC2 = Instance.new("UICorner")
    LBC2.CornerRadius = UDim.new(1, 0)
    LBC2.Parent = LuarmorBadge
    local LBP2 = Instance.new("UIPadding")
    LBP2.PaddingLeft = UDim.new(0, 6)
    LBP2.PaddingRight = UDim.new(0, 6)
    LBP2.Parent = LuarmorBadge
    local LBL2 = Instance.new("TextLabel")
    LBL2.Font = Enum.Font.GothamBold
    LBL2.Text = "LUARMOR"
    LBL2.TextColor3 = Color3.fromRGB(220, 210, 255)
    LBL2.TextSize = 9
    ...
    local function submitKey(key)
        ...
        task.spawn(function()
            local valid = luarmorCheckKey(key)
            checking = false
