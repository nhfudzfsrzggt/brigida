...
function Chloex:Window(GuiConfig)
    ...
    -- Garis pemisah di sidebar antar tab.
    -- Penggunaan: GuiFunc:Separator()
    -- ══════════════════════════════════════════════════════════════
    ...
    local Tabs = TabsModule(
        ...
        getIconId, CircleClick, function() end, {}
    )
    ...
    GuiFunc.AddTab = function(self, TabConfig)
        ...
        -- Ambil opsi fitur baru sebelum diteruskan
        local useColumns  = TabConfig.Columns == true  -- ✦ FITUR: multi-column
        local useSearch   = TabConfig.Search  == true  -- ✦ FITUR: page search
        -- Hapus agar tidak mengganggu TabsModule asli
        TabConfig.Columns = nil

        local Sections = origAddTab(self, TabConfig)

        -- ──────────────────────────────────────────────────────────
        -- ✦ FITUR BARU 2: SUBPAGE
        -- Nested page di dalam Tab dengan dua kolom Kiri/Kanan.
        --
        -- Penggunaan:
        --   local Tab = UI:AddTab({ Title = "Combat" })
        --   local Sub = Tab:AddSubPage({ Title = "Aimbot" })
        --   local S   = Sub:AddSection({ Title = "Settings", Side = "Left" })
        --   S:AddToggle({ Title = "Enable", Callback = ... })
        -- ──────────────────────────────────────────────────────────
        local _subPages    = {}
        local _subBarFrame = nil
        local _tabPageRef  = nil  -- Frame halaman tab ini di LayersFolder

        -- Dapatkan frame halaman tab saat ini (anak terakhir LayersFolder)
        ...
        -- Buat bar sub-tab di atas halaman tab
        local function _ensureSubBar(tabPage)
            ...
            -- Semua scroll asli di tabPage digeser ke bawah bar
            for _, child in ipairs(tabPage:GetChildren()) do
                if child ~= _subBarFrame and child:IsA("ScrollingFrame") then
                    child.Visible = false  -- disembunyikan, diganti kolom SubPage
                end
            end
        end
        ...
        function Sections:AddSubPage(cfg)
            ...
            task.defer(function()
                ...
                -- Frame konten SubPage (posisi di bawah bar)
                ...
                -- ✦ FITUR BARU 3: MULTI-COLUMN per SubPage
                -- AddSection dengan Side = "Left" atau "Right"
                function Sub:AddSection(sectionCfg)
                    ...
                    -- Buat section frame langsung di kolom yang tepat
                    ...
                    -- Buat Items API yang kompatibel dengan AddSection asli
                    -- dengan mendelegasikan ke Sections:AddSection dari TabsModule
                    -- Approach: buat section dummy, ambil Items-nya, re-parent
                    local dummyItems = Sections:AddSection(sectionCfg)

                    -- Pindahkan frame section yang baru dibuat ke targetCol
                    task.defer(function()
                        -- Cari frame section terbaru di scroll asli tab
                        for _, child in ipairs(tabPage:GetChildren()) do
                ...
                -- Aktifkan sub-page pertama secara default
                if #_subPages == 1 then
                    Sub:SetActive(true)
                end

                -- Kembalikan Sub setelah defer selesai
                cfg._resolved = Sub
            end)

            -- Return placeholder; gunakan task.defer agar frame sudah exist
            local placeholder = {}
        ...
        -- ──────────────────────────────────────────────────────────
        -- ✦ FITUR BARU 4: PAGE SEARCH
        -- Searchbox di atas section list, highlight elemen yang cocok.
        --
        -- Penggunaan:
        --   local Tab = UI:AddTab({ Title = "Combat", Search = true })
        -- ──────────────────────────────────────────────────────────
        if useSearch then
            task.defer(function()
                local tabPage = _getTabPage()
                ...
                -- Cari ScrollingFrame konten di halaman tab ini
                ...
                -- Geser scroll ke bawah untuk beri ruang searchbox (36px)
        ...
        -- ──────────────────────────────────────────────────────────
        -- ✦ FITUR BARU 3 (standalone): MULTI-COLUMN tanpa SubPage
        -- Gunakan AddSection dengan Column = "Left" / "Right"
        --
        -- Penggunaan:
        --   local Tab = UI:AddTab({ Title = "Settings", Columns = true })
        --   local L = Tab:AddSection({ Title = "Kiri",  Column = "Left" })
        --   local R = Tab:AddSection({ Title = "Kanan", Column = "Right" })
        -- ──────────────────────────────────────────────────────────
        if useColumns then
            ...
            task.defer(function()
                ...
                -- Sembunyikan scroll asli
                local origScroll = nil
                for _, child in ipairs(tabPage:GetChildren()) do
                    if child:IsA("ScrollingFrame") then
                        origScroll = child
                        child.Visible = false
            ...
            -- Wrap AddSection untuk redirect ke kolom yang benar
            ...
            Sections.AddSection = function(self2, SectionConfig)
                ...
                -- Pindahkan section frame ke kolom yang tepat
                task.defer(function()
                ...
                -- Wrap element functions untuk config system (sama seperti aslinya)
                local _itemCount = 0
        ...
        -- ──────────────────────────────────────────────────────────
        -- Wrap AddSection untuk mode biasa (bukan Columns)
        -- Mempertahankan AddColorpicker + Config System
        -- ──────────────────────────────────────────────────────────
        local origAddSection = Sections.AddSection
        Sections.AddSection = function(self2, SectionConfig)
            local Items = origAddSection(self2, SectionConfig)
